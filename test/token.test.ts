...
describe("AssemblyToken constructor errors", () => {
  describe("throws AssemblyNoTokenError for missing tokens", () => {
    it("undefined", () => {
      expect(
        () => new AssemblyToken({ apiKey: TEST_API_KEY, token: undefined })
      ).toThrow(AssemblyNoTokenError);
    });

    it("null", () => {
      expect(
        () => new AssemblyToken({ apiKey: TEST_API_KEY, token: null })
      ).toThrow(AssemblyNoTokenError);
    });

    it("empty string", () => {
      expect(
        () => new AssemblyToken({ apiKey: TEST_API_KEY, token: "" })
      ).toThrow(AssemblyNoTokenError);
    });
  });
  ...
  describe("throws AssemblyInvalidTokenError for invalid tokens", () => {
    it("non-string value", () => {
      expect(
        () => new AssemblyToken({ apiKey: TEST_API_KEY, token: 123 })
    ...
    it("wrong API key", () => {
      expect(
        () =>
          new AssemblyToken({ apiKey: "wrong-api-key", token: CLIENT_TOKEN })
      ).toThrow(AssemblyInvalidTokenError);
    });
