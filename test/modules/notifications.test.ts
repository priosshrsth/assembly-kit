...
describe("NotificationsResource", () => {
  ...
  it("markRead() sends POST to v1/notifications/:id/read", async () => {
    const { client, calls } = createTestClient([{ body: {}, status: 200 }]);

    await client.notifications.markRead("notif-1");

    const call = callAt(calls);
    expect(call.method).toBe("POST");
    expect(call.url).toBe(`${BASE_URL}/v1/notifications/notif-1/read`);
  });

  it("markUnread() sends POST to v1/notifications/:id/unread", async () => {
    const { client, calls } = createTestClient([{ body: {}, status: 200 }]);

    await client.notifications.markUnread("notif-1");

    const call = callAt(calls);
    expect(call.method).toBe("POST");
    expect(call.url).toBe(`${BASE_URL}/v1/notifications/notif-1/unread`);
  });
});
