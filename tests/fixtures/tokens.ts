...
// ---------------------------------------------------------------------------
const encryptPayload = (apiKey: string, payload: Record<string, string>): string =>
  encryptTokenString({ apiKey, plaintext: JSON.stringify(payload) });
...
const padPayload = (
  ...
  if (currentLen > targetLen) {
    throw new Error(`Base payload is ${currentLen} bytes, cannot shrink to ${targetLen}`);
  }
