...
import { HexColorSchema, TokenPayloadSchema } from "src/schemas";
...
describe("CustomFieldTypeSchema", () => {
  it("accepts all valid types", () => {
    const types = ["address", "email", "phoneNumber", "text", "number", "url", "multiSelect"];
...
describe("CustomFieldEntityTypeSchema", () => {
  ...
  it("rejects unknown entity type", () => {
    expect(CustomFieldEntityTypeSchema.safeParse("workspace").success).toBe(false);
  });
});
...
describe("CompaniesResponseSchema", () => {
  ...
  it("accepts null data", () => {
    expect(CompaniesResponseSchema.safeParse({ data: null }).success).toBe(true);
  });
});
...
describe("NotificationsResponseSchema", () => {
  ...
  it("accepts null data", () => {
    expect(NotificationsResponseSchema.safeParse({ data: null }).success).toBe(true);
  });
});
...
describe("ClientCreateRequestSchema", () => {
  ...
  it("rejects missing required fields", () => {
    expect(ClientCreateRequestSchema.safeParse({ email: "jane@example.com" }).success).toBe(false);
  });
});

describe("CompanyCreateRequestSchema", () => {
  it("accepts a valid create request", () => {
    expect(CompanyCreateRequestSchema.safeParse({ name: "Acme" }).success).toBe(true);
  });
