...
export interface Transport {
  ...
  delete(path: string, opts?: RequestOpts): Promise<void>;
}
...
const mapHttpError = async (error: HTTPError): Promise<AssemblyError> => {
  ...
  return createErrorForStatus(response.status, error, details, response.headers.get("Retry-After"));
};
...
export const createTransport = (options: TransportOptions): Transport => {
  ...
  return {
    ...
    delete: async (path: string, opts?: RequestOpts): Promise<void> => {
      try {
        await api.delete(stripLeadingSlash(path), {
          searchParams: opts?.searchParams,
        });
      } catch (error) {
        if (error instanceof HTTPError) {
